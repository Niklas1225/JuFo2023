...
def AutoClassImpute(data, pred_label, cellwise_norm=True, log1p=True, num_cluster=9,
                    encoder_layer_size=[128], dropout_rate=0.1, classifier_weights=[.5, .4, .1],
    ...
    classifier_weights: 'list'. Default: [.5, .4, .1].
    ...
    AC.set_reduce_lr(lr)
    AC.set_reg(reg)
    AC.set_classifier_weights(classifier_weights)
    ...
    models = []
    loss_history = []
    if classifier_weights[0]+classifier_weights[1] == 0:
...
class AutoClass(object):
    ...
    def set_label(self, y):
        self.pred_label = to_categorical(y)
        self.n_labels = self.pred_label.shape[-1]
    ...
    def set_classifier_weights(self, w):
        self.classifier_weight = w[0]
        self.prediction_weight = w[1]
        self.decoder_weight = w[2]
    ...
    def create_model(self):
        ...
        self.prediction_layer = tf.keras.layers.Dense(self.n_labels, \
                                                      activation=tf.nn.softmax,
                                                      name='prediction',
                                                      kernel_regularizer= \
                                                          tf.keras.regularizers.l2(self.reg))(self.neck_layer)

        if (self.classifier_weight+self.prediction_weight == 0) or (self.n_cluster <= 1):
            self.model = tf.keras.Model(inputs=self.input_layer,
                                        outputs=self.decode_layer)
        else:
            self.model = tf.keras.Model(inputs=self.input_layer,
                                        outputs=[self.classifier_layer, self.decode_layer, self.prediction_layer])
        ...
        if (self.classifier_weight+self.prediction_weight == 0) or (self.n_cluster <= 1):
            ...
            self.model.compile(loss={'classification': 'categorical_crossentropy',
                               ...
                               loss_weights={'classification': self.classifier_weight,
                                             'reconstruction': self.decoder_weight,
