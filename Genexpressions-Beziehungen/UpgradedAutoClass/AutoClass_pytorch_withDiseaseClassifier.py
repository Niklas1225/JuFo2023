...
class AutoClass(nn.Module):
    def __init__(self, ncell, ngene, encoder_layer_size, dropout_rate, n_clusters, n_classes):
        ...
        #disease classifer
        self.DC_layer = nn.Linear(encoder_layer_size[-1], self.n_classes)
        self.DC_activation = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        x_DC = self.DC_activation(self.DC_layer(x))
        
        return x_imp, x_classified, x_DC
...
def training(AC, data, dummy_label, pred_label, lr, batch_size, epochs, classifier_weights, reg, reduce_lr, early_stopping, verbose):
    ...
    for i in range(len(data)):
        train_data.append([data[i], (dummy_label[i], pred_label[i])])
    ...
    for epoch in range(epochs):
        ...
        with torch.set_grad_enabled(True):
            AC.train()
            for inp_data, (labels, pred_labels) in trainloader:
                optimizer.zero_grad()
                out_imp, out_classified, out_DC = AC(inp_data.to(device))
                ...
                loss_DC = pytorch_categorical_ce(pred_labels.to(device), out_DC)
                loss = classifier_weights[-1] * loss_imp + classifier_weights[0] * loss_clusters + classifier_weights[1] * loss_DC
                loss.backward()
                optimizer.step()
        ...
        with torch.no_grad():
            AC.eval()
            for inp_data, (labels, pred_labels) in valloader:
                optimizer.zero_grad()
                out_imp, out_classified, out_DC = AC(inp_data.to(device))
                loss_imp = criterion_decoder(out_imp, inp_data.to(device))
                loss_clusters = pytorch_categorical_ce(labels.to(device).argmax(-1), out_classified)
    ...
    imp = []
    with torch.no_grad():
        for inp_data, (labels, pred_labels) in loader:
            AC.eval()
            optimizer.zero_grad()
            out_imp, out_classified, out_DC = AC(inp_data.to(device))
...
def autoClassImpute(data, pred_label, encoder_layer_size=[128], num_clusters=9, dropout_rate=0.1, classifier_weights=[.1, .8, .1], truelabel=[], cellwise_norm=True, log1p=True, reg=0.000, batch_size=32, epochs=300, verbose=False, npc=15, early_stopping=30, reduce_lr=15, lr=0.001):
    ...
    for n_cluster in num_clusters:
        ...
        AC = AutoClass(ncell, ngene, encoder_layer_size, dropout_rate, n_cluster, len(np.unique(pred_label)))
        imp = training(AC, data, dummy_label, pred_label, lr, batch_size, epochs, classifier_weights, reg, reduce_lr, early_stopping, verbose)
        imps = imps + imp
